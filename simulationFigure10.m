...
sumSE_corr = zeros(length(ADCresolutions),2);
sumSE_uncorr = zeros(length(ADCresolutions),2);
...
for n = 1:nbrOfRealizations
    ...
    for k = 1:length(ADCresolutions)
        
        ADCresolution = ADCresolutions(k);
...
avgSE_MMSE_uncorr = sumSE_uncorr(:,1)./K';
avgSE_MMSE_corr = sumSE_corr(:,1)./K';
avgSE_MR_uncorr = sumSE_uncorr(:,2)./K';
avgSE_MR_corr = sumSE_corr(:,2)./K';


%% Plot Figure 10
figure;
hold on; box on;
plot(ADCresolutions,avgSE_MMSE_uncorr,'bo--','LineWidth',1);
plot(ADCresolutions,avgSE_MMSE_corr,'ro-','LineWidth',1);
plot(ADCresolutions,avgSE_MR_uncorr,'b--','LineWidth',1);
plot(ADCresolutions,avgSE_MR_corr,'k-','LineWidth',1);
